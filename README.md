...
- La configuracion biomecanica de cada tecnica se guarda en `biomechanicsConfig` con `referenceMediaAssetId`, `referenceMotionProfile`, `focusPoints`, `angleChecks`, `trajectoryChecks`, `keyEvents`, `orientationPolicy` y `coachNotes`.
...
- Los `angleChecks` ya pueden anclarse a un evento concreto o a una ventana entre eventos, y `trajectoryChecks` permite expresar recorridos biomecanicos como la cadera entre penúltimo apoyo y despegue dejando listo el JSON canónico para la futura comparación atleta-vs-pro.
