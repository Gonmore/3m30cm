...
- El calculo de altura por `CENTER_OF_MASS` y `RIM_REFERENCE` en la referencia profesional del admin web ya no usa `SETUP` ni `DIP` como baseline: ahora exige `TOE_OFF` y mide la elevacion hasta `APEX` para reducir sesgo por traslacion/rotacion de camara.
- La deteccion automatica de `TOE_OFF` en el editor biomecanico del admin se ajusto para priorizar el ultimo frame posterior a `DIP` donde ambas puntas (`LEFT_FOOT_INDEX` y `RIGHT_FOOT_INDEX`) siguen en apoyo antes del primer frame airborne.
...
- La version release preparada en el repo queda en `2.0.0` (`versionCode 200`) y el artefacto esperado sigue siendo `apps/mobile2/android/app/build/outputs/apk/release/app-release.apk`.
- Antes de distribuir la APK `2.0.0`, conviene ejecutar primero tu deploy manual con `./deploy.sh` para que produccion ya tenga aplicadas las migraciones de Prisma y los campos nuevos que consume la app.
...
- La configuracion biomecanica de cada tecnica se guarda en `biomechanicsConfig` con `referenceMediaAssetId`, `referenceMotionProfile`, `focusPoints`, `pointChecks`, `angleChecks`, `trajectoryChecks`, `keyEvents`, `orientationPolicy` y `coachNotes`.
- Cada `keyEvent` ya puede persistir `source`, `confidence` y `detector` para diferenciar sugerencias automáticas de overrides manuales dentro del portal admin.
- Los `keyEvents` ya guardan `frameIndex` explicito ademas de `frameHint`, para que el admin pueda volver al frame exacto desde la timeline y desde los formularios inferiores sin depender solo del texto descriptivo.
- Los `angleChecks` ya pueden anclarse a un evento concreto o a una ventana entre eventos, `pointChecks` permite comparar landmarks como la cadera por eje y por evento/ventana, y `trajectoryChecks` permite expresar recorridos biomecanicos como la cadera entre penúltimo apoyo y despegue dejando listo el JSON canónico para la futura comparación atleta-vs-pro.
- `packages/shared` ya define el contrato futuro del analisis del atleta como `poseSequence + detectedEvents + measuredChecks`, para que la siguiente capa de subida y comparación del video del usuario parta de un esquema compartido.
...
- La misma vista `Técnica` ahora tambien puede analizar un video del atleta dentro de la app: extrae landmarks en un `WebView`, reutiliza la deteccion de eventos y las mediciones de referencia de `apps/web`, prioriza `CENTER_OF_MASS` contra el suelo en `SETUP` y `APEX`, usa `FLIGHT_TIME` solo como metodo secundario y deja disponible el JSON de analisis para depuracion o futura subida al backend.
