...
- El portal web ya incluye un editor biomecanico visual sobre esa referencia: scrubber por frame, overlay de landmarks/esqueleto, alta visual de puntos clave, construccion visual de angulos y marcado de eventos sobre timeline.
...
- La configuracion biomecanica de cada tecnica se guarda en `biomechanicsConfig` con `referenceMediaAssetId`, `referenceMotionProfile`, `focusPoints`, `angleChecks`, `keyEvents` y `coachNotes`.
