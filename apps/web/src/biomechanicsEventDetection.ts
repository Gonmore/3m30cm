...
export function detectTechniqueKeyEvents(landmarks: TechniqueProLandmarks | null | undefined): AutoDetectedTechniqueKeyEvent[] {
  ...
  const contactProminence = Math.max(Math.max(...leftContactSeries, ...rightContactSeries) - Math.min(...leftContactSeries, ...rightContactSeries), 0.01) * 0.03;
  const contactCandidates = [
    ...findLocalMaxima(leftContactSeries, 0, toeOffIndex, 3, contactProminence),
    ...findLocalMaxima(rightContactSeries, 0, toeOffIndex, 3, contactProminence),
  ]
    .filter((frameIndex, index, list) => list.indexOf(frameIndex) === index)
    .sort((left, right) => left - right)
    .filter((frameIndex) => anyGroundedFlags[frameIndex] ?? false);

  const lastContactIndex = contactCandidates[contactCandidates.length - 1] ?? takeOffIndex;
  const penultimateContactIndex = contactCandidates[contactCandidates.length - 2] ?? null;
  const antepenultimateContactIndex = contactCandidates[contactCandidates.length - 3] ?? null;
